...
// import { productsData } from '../Data/ProductsData'; // ✅ Ensure this is uncommented and valid
...
export class Searching extends Component {
  ...
  this.setState((prevState) => ({
    ...
    searchTerm: prevState.isExpanded ? '' : prevState.searchTerm,
  }));
};

  getFilteredProducts = () => {
    const { searchTerm, productsData } = this.state;
  ...
  render() {
    const { searchTerm, isExpanded } = this.state;
    ...
    return (
      ...
      <div className="search-container">
        ...
        onClick={this.handleToggle}>
          <CiSearch data-tooltip-id="search-tooltip"
          data-tooltip-content="Click to search" className="search-icon"  />
          <Tooltip id='search-tooltip' className='custom-tooltip' place='top' effect="solid" />
        {isExpanded && (  <input
            type="text"
            ...
            {filteredProducts.map((product) => (
              ...
              >
                ...
                <img
                  ...
                  alt={`${product.category} ${product.brand}`}
                  width="50"
                />
                <span>{product.category}{product.brand}{product.info}</span>
              </Link>
            ))}
          </div>
        )}
